...
import { View, Text, StyleSheet, TextInput, ScrollView, Pressable, Alert, Platform, Modal, PanResponder, Animated } from 'react-native';
...
import { WorkoutTemplate, TemplateExercise, SessionExercise, WorkoutSession, ProgramWorkout, Program } from '@fitness-tracker/domain';
...
export default function ProgramBuilderScreen() {
      ...
      onPanResponderRelease: (e, gestureState) => {
        ...
        if (wk && activeProgram) {
          ...
          if (layout) {
            ...
            const otherWorkouts = activeProgram.workouts.filter((w: ProgramWorkout) => w.id !== wk.id);
            ...
            if (dayLayout) {
              ...
              insertIndex = Math.max(0, Math.min(targetDayWorkouts.length, Math.round(relativeY / 60)));
            }
            ...
            const reorderedTargetDayWorkouts = targetDayWorkouts.map((w: ProgramWorkout, idx: number) => ({
              ...w,
              order: idx,
            }));
            ...
            handleChange({
              workouts: [
                ...otherWorkouts.filter((w: ProgramWorkout) => !(w.week === selectedWeek && w.dayOfWeek === targetDay)),
  ...
  const handleSelectTemplate = (templateId: string) => {
    ...
    const newWorkout = {
      ...
      week: selectedWeek,
      dayOfWeek: activeDay,
      order: activeProgram.workouts.filter((w: ProgramWorkout) => w.week === selectedWeek && w.dayOfWeek === activeDay).length,
    };
    handleChange({
    ...
    <View style={{ flex: 1, backgroundColor: theme.colors.background }}>
        ...
        options={{ 
          ...
          headerLeft: () => (
            <Pressable onPress={handleCancel} hitSlop={15} style={{ paddingLeft: 8 }}>
              <Text style={{ color: theme.colors.accent, fontFamily: 'SpaceGrotesk_700Bold', fontSize: 15 }}>
                Cancel
              </Text>
            </Pressable>
          ),
          headerRight: () => (
            <Pressable onPress={handleSave} hitSlop={15} style={{ paddingRight: 8 }}>
              <Text style={{ color: theme.colors.primary, fontFamily: 'SpaceGrotesk_700Bold', fontSize: 15 }}>
                Save
              </Text>
            </Pressable>
          ),
      ...
      <ScrollView contentContainerStyle={styles.content} scrollEnabled={scrollEnabled}>
        ...
        {[1, 2, 3, 4, 5, 6, 7].map((day) => {
          const dayWorkouts = activeProgram.workouts.filter((w: ProgramWorkout) => w.week === selectedWeek && w.dayOfWeek === day);
          const draggingWorkout = activeDragId ? activeProgram.workouts.find((w: ProgramWorkout) => w.id === activeDragId) : null;
              ...
              {dayWorkouts.map((w: ProgramWorkout) => {
                ...
                return (
                  ...
                  >
                    ...
                    <Text style={[styles.workoutName, { color: theme.colors.text, flex: 1, marginLeft: 8 }]}>
                      {template?.name || 'Unknown Template'}
                    </Text>
                    <View style={styles.workoutActions}>
                      <Pressable onPress={() => handleStartTemplate(template, activeProgram.id)} hitSlop={8}>
                        <Text style={[styles.startText, { color: theme.colors.primary }]}>Start</Text>
                      </Pressable>
                      <Pressable onPress={() => router.push(`/programs/template-builder?programId=${activeProgram.id}&templateId=${template?.id}&dayOfWeek=${day}&week=${selectedWeek}`)} hitSlop={8}>
                        <Text style={[styles.editText, { color: theme.colors.text }]}>Edit</Text>
                      </Pressable>
                      <Pressable onPress={() => removeWorkout(w.id)} hitSlop={8}>
                        <Text style={[styles.removeText, { color: theme.colors.accent }]}>Remove</Text>
                      </Pressable>
                    </View>
                  </Animated.View>
                );
      ...
      <Modal
        ...
        transparent
        animationType="slide"
        onRequestClose={() => { setActiveDay(null); setModalMode(null); }}
      >
        <Pressable style={styles.modalOverlay} onPress={() => { setActiveDay(null); setModalMode(null); }}>
          <Pressable
          ...
          >
            {modalMode === 'options' ? (
              <>
                ...
                <Pressable
                  ...
                  onPress={() => {
                    ...
                    setActiveDay(null);
                    setModalMode(null);
                    router.push(`/programs/template-builder?programId=${activeProgram.id}&dayOfWeek=${day}&week=${selectedWeek}`);
                  }}
                >
                ...
                <Pressable
                  style={[styles.modalBtn, { backgroundColor: 'transparent', borderWidth: 1, borderColor: theme.colors.border, borderRadius: theme.radius.md }]}
                  onPress={() => { setModalMode('templates'); setPickerTab('templates'); }}
                >
                  <Text style={[styles.modalBtnText, { color: theme.colors.text, ...theme.typography.button }]}>
              ...
              <>
                ...
                <View style={styles.tabContainer}>
                  <Pressable
                    style={[styles.tabButton, pickerTab === 'templates' && { borderBottomColor: theme.colors.primary }]}
                    onPress={() => setPickerTab('templates')}
                  >
                    <Text style={[styles.tabButtonText, { color: pickerTab === 'templates' ? theme.colors.text : theme.colors.muted }]}>
                      Templates
                    </Text>
                  </Pressable>
                  <Pressable
                    style={[styles.tabButton, pickerTab === 'history' && { borderBottomColor: theme.colors.primary }]}
                    onPress={() => setPickerTab('history')}
                  >
                    <Text style={[styles.tabButtonText, { color: pickerTab === 'history' ? theme.colors.text : theme.colors.muted }]}>
                      History
                    </Text>
                  </Pressable>
                </View>
                ...
                {pickerTab === 'templates' ? (
                  <ScrollView style={styles.templateList} showsVerticalScrollIndicator={false}>
                    {templates.map(t => (
                      ...
                      >
                        <Text style={[styles.templateItemName, { color: theme.colors.text }]}>{t.name}</Text>
                    ...
                    {templates.length === 0 && (
                      <Text style={{ color: theme.colors.muted, textAlign: 'center', marginVertical: 20 }}>
                  ...
                  <ScrollView style={styles.templateList} showsVerticalScrollIndicator={false}>
                    {sessions.map(s => (
                      ...
                      >
                        <Text style={[styles.templateItemName, { color: theme.colors.text }]}>{s.name}</Text>
                        <Text style={[styles.templateItemCount, { color: theme.colors.muted }]}>
                          {s.completedAt ? new Date(s.completedAt).toLocaleDateString() : ''} • {s.exercises.length} exercise{s.exercises.length !== 1 ? 's' : ''}
                        </Text>
                      </Pressable>
                    ))}
                    {sessions.length === 0 && (
            ...
            <Pressable
              ...
              onPress={() => { setActiveDay(null); setModalMode(null); }}
            >
              <Text style={{ color: theme.colors.accent, fontFamily: 'SpaceGrotesk_700Bold' }}>Cancel</Text>
            </Pressable>
          </Pressable>
        </Pressable>
      </Modal>
