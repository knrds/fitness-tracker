...
export default function ExerciseDetailScreen({ exerciseId, onBack }: { exerciseId?: string; onBack?: () => void } = {}) {
  const params = useLocalSearchParams<{ id: string }>();
  const id = exerciseId ?? params.id;
    ...
    <View style={{ flex: 1, backgroundColor: theme.colors.background }}>
      {onBack ? (
        <Pressable onPress={onBack} accessibilityRole="button"
          accessibilityLabel={language === 'de' ? 'Zurück zur Übungsauswahl' : 'Back to exercise selection'}
          style={{ minHeight: 48, padding: 12, flexDirection: 'row', alignItems: 'center', gap: 12 }}>
          <Ionicons name="arrow-back" size={24} color={theme.colors.primary} />
          <Text style={{ color: theme.colors.text, flexShrink: 1 }}>{exercise.name}</Text>
        </Pressable>
      ) : <Stack.Screen
        options={{
          title: exercise.name,
        ...
        <View style={styles.detailsContainer}>
          ...
          <View style={styles.infoRow}>
            <View style={styles.infoCard}>
              ...
              <Text style={styles.infoValue}>{(exercise.equipmentOptions ?? [exercise.equipment]).map(formatEquipment).join(', ')}</Text>
            </View>
            <View style={styles.infoCard}>
