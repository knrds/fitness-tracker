...
| Notification Preferences & Rest Timer (07.02 / 07.04) | Einstellungen -> Benachrichtigungen öffnen, Kanäle toggeln; Rest-Timer starten und abbrechen | Granulare Schalter (Rest-Timer, Reminder, Coach, Marketing); Rest-Timer Notification wird bei Cancel sofort annulliert; Lockscreen enthält 0 Gesundheits-/Coach-Daten | Automatisiert in `notifications.test.ts`, `notificationPreferenceStore.test.ts`, `localNotificationService.test.ts` (13 Tests PASS); Natives Push-Token `PHYSICAL_DEVICE_REQUIRED` |
| Contextual Permission Pre-Prompts (06.06) | Pre-Prompt Modal für Notifications, Mikrofon oder Fotos aufrufen | Transparente Erklärung des konkreten Nutzens; 'Nicht jetzt' schließt das Modal ohne Systemdialog; keine Start-Wall | Automatisiert in `ContextualPermissionModal.test.tsx` (3 Tests PASS) |
| Coach Circuit Breaker (03.10) | 3 aufeinanderfolgende 5xx/429 Netzwerkfehler im Coach provozieren | Circuit Breaker wechselt auf `OPEN`; nachfolgende Anfragen fail-fast ohne Provider-Aufruf oder Kosten; Cooldown läuft | Automatisiert in `coachCircuitBreaker.test.ts` (7 Tests PASS); Verteilter serverseitiger State `ASTRA_REQUIRED` |
| Remote Config & Kill Switch (08.04) | Feature-Status via `RemoteConfigService` abfragen | Schema-validiert; Fallback auf Fail-Safe Defaults bei Timeout/Fehler; getötetes Feature meldet `isFeatureKilled() === true` | Automatisiert in `remoteConfigService.test.ts` (6 Tests PASS); Backend-Auswahl `ASTRA_REQUIRED` |
| Support & Diagnostic Path (08.05) | Einstellungen -> Hilfe & Support öffnen | Modal öffnet in <= 2 Taps; FAQ einsehbar; Diagnose-Mail enthält nur technische Plattformdaten, 0 PII / Workouts / Prompts | Automatisiert in `SupportFeedbackModal.test.tsx` (3 Tests PASS); Offizielle Mail `USER_ACTION_REQUIRED` |
| Review Prompt Policy (08.06) | `shouldRequestReview` nach Fehler vs. nach PR prüfen | Nach Fehler/Crash mindestens 48h Cooldown; nach PR/Streak erlaubt sofern Cooldown (60d) und Jahreslimit (3/Jahr) eingehalten | Automatisiert in `reviewPromptPolicy.test.ts` (6 Tests PASS) |
| Large Text Font Scaling (09.05) | UI-Buttons und Modals bei Font-Skalierung rendern | `maxFontSizeMultiplier` schützt vor Text-Clipping; Flex-Wrap aktiv | Automatisiert in `accessibilityScalingRegression.test.tsx` (2 Tests PASS); iOS/Android `PHYSICAL_DEVICE_REQUIRED` |
| Core Product Analytics Wiring (08.03) | Onboarding und 1. / 2. Workout abschließen | Events `onboarding_started`, `onboarding_completed`, `first_workout`, `second_workout` gefeuert; Payloads frei von Gesundheits-/Workout-Rohdaten | Automatisiert in `monetizationAnalytics.test.ts` (4 Tests PASS) |
| Coach Data Intelligence (WP-03 / S6) | Im Coach spezifische Fragen zu Profil, Übungsverlauf, PRs, Metriken und Programmen stellen | 15 selektive Selektoren liefern relevante Details; FREE blockiert via `COACH_LOCKED`; PRO reduzierter Snapshot; COACH Vollkontext; 0 PII; Notizen max. 120 Zeichen; max. 5 Übungen, max. 3 Templates | Automatisiert in `coachContextBuilder.test.ts` (49 Tests PASS); Server RLS `ASTRA_REQUIRED` |
| Program Editor DnD Parity (WP-09) | Im Program Editor ein Workout greifen, über Tage ziehen und ablegen | Card Elevation 25, zIndex 10000, 103% Skalierung, Day-Hover Badge („Hier ablegen“); Haptik beim Drop; saubere Reorder-Geometrie ohne Duplikate/Verlust | Automatisiert in `programReorder.test.ts` (21 Tests PASS); Native Safari/Device Sichtprüfung |
| Home Workout Preview & Schedule (Core UX) | Auf dem Home-Screen das Widget für das heutige Programm-Workout antippen | Reine Schedule-Evaluation (Rest Day vs. Workout); Tap öffnet `WorkoutPreviewModal` mit Übungen, Sätzen, Wdh & Targets; kein blinder Sofort-Start | Automatisiert in `programSchedule.test.ts` (15 Tests PASS) & `WorkoutPreviewModal.test.tsx` (4 Tests PASS) |
