...
| Age / Geburtstag UI | Profil-Tab öffnen | Kein Glitzer-Icon am Alters-Badge; saubere Zahl mit 'Jahre' / 'years old' | Automatisiert via Typcheck/Profile-Tests; visuelle Safari-Prüfung |
| XP / Level Rebalancing | Vorherigen XP/Levelstand notieren, 1 normales Workout absolvieren | Kein Sprung über mehrere frühe Level (L1->2 erfordert 1.000 XP; Workout liefert ~120-160 XP); Idempotenz-Schutz | Automatisiert in `levelProgression.test.ts` und `achievementStore.test.ts` |
